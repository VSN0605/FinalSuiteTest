...
class SugarWebServiceImplv4_1 extends SugarWebServiceImplv4
{
    ...
    public function weekly_leads($session)
    {    
        $error = new SoapError();
        $GLOBALS['log']->fatal('weekly_leads');
        if(!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session','','','',$error ))
        {
            $GLOBALS['log']->fatal('End:SugarWebServiceImplv4->lat_get_mobile_modules');
            $error->set_error('invalid login');
        ...
        return array('status' => 'success', 'message' => 'function to get weekly leads','weekly_leads' => $total);
